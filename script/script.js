const imageSets = [
  ...
  const displayImage = (list) => {
    const imgContainer = document.getElementById("content-left");
    const allImg = list.map (item => {
    return `<a href="slide.html?id=${item.id}">
            <div class="images1">
              <img src=${item.images[0]} alt=${item.name} class="img-entry" />
              <div class="img-info">
                <p class="image-name">${item.title} - ${item.name}</p>
                <p label="see-more" class="item-link">See More</p>
              </div>
            </div>
            </a>`
    })
    imgContainer.innerHTML = allImg.join("");
  }

  window.onload = function() {
    displayImage(imageSets);
  }
...
const toggleTheme = () => {
  ...
  const p = document.querySelectorAll(".img-info");
...
submitBtn.addEventListener("click", (e) => {
  ...
  const name = document.querySelector('inout[name = "name"]').value;
  const imageUrl = document.querySelectorAll('input[name = "url"]').value;
  const title = document.querySelector('input[name = "title"]').value;

  const newWork = document.createElement("div");
  newWork.innerHTML = `<a href="slide.html">
  <div class="images1">
    <img src=${item.images[0]} alt=${item.name} class="img-entry" />
    <div class="img-info">
      <p class="image-name">${item.title} - ${item.name}</p>
  ...
  const imgContainer = document.querySelector(".content-left")
  imgContainer.insertBefore(newWork,imgContainer.firstChild);

  document.querySelector('inout[name = "name"]').value = "";
  document.querySelectorAll('input[name = "url"]').value = "";
