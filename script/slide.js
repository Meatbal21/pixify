const imageSets = [
 ...
 window.onload = function(){
    ...
    const currentImg = imageSets.filter(item => item.id == currentId)
    const img = `<div class="img-title">
              <p>${currentImg[0].title}</p>
              <p class="img-name">${currentImg[0].name}</p>
              <div class="img">
                  <img src=${currentImg[0].images[2]} alt=${currentImg[0].name} class="img-sequence" id="imgSequence"/>  
              </div>
          </div>`
...
//const imagePage = (list) => {
 //   const imgPage = document.getElementById("image-page");
  //  const imgSlide = list.map (item => {
    //return `<div class="img-title">
      //          <p>${item.title}</p>
        //        <p class="img-name">${item.name}</p>
          //      <div class="img">
            //        <img src=${item.images[0]} alt=${item.name} class="img-sequence" id="imgSequence"/>  
              //  </div>
            //</div>`
    //});
    //imgPage.innerHTML = imgSlide.join("");
    //imgPage.innerHTML = imgSlide[0];
    
    //const currentImage = imgSlide.reduce((accumulator, currentImg) => {
      //  console.log('${currentImg}');
        //return accumulator + currentImg;
    //},"");
...
nextBtn.addEventListener('click', () => {
       const imageTag = document.getElementById('imgSequence')
       currentImage++;
       if(currentImage >= lastImage){
           currentImage = 4;
        }
        
       imageTag.src = currentImg[0].images[currentImage];
});
...
//window.onload = function(){
  //  imagePage(imageSets);
    //};
// var sliderImg = document.querySelector (".image-sequence");

// var i = 0;

// function prev() {
  
//    if(i <= 0) i = currentImg.images.length;
//    i--;
//    return setImg();
// }

// function next() {
//    if (i >= currentImg.images.length -1) i = -1;
//    i++;
//     return setImg();
// }

// function setImg() {
//    return sliderImg.setAttribute('src',currentImg[0].images[i]);
// }
